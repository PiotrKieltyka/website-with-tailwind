...
@Component({
  ...
  template: `
    <ng-container>
      <div
        class="w-full h-full flex flex-col items-center justify-center text-center hover:scale-105 transition-transform"
      >
        ...
        <ng-template [ngIf]="link">
          <a
            class="text-sm font-light w-3/4 md:w-1/4 h-8 my-4 mt-4 grid items-center justify-center bg-white hover:shadow-md hover:shadow-indigo-600 hover:bg-gradient-to-br hover:from-indigo-400 hover:to-indigo-600 hover:text-white hover:font-normal transition-all shadow-inner shadow-indigo-300 rounded-lg"
            href="{{ link }}"
            >Learn more</a
          >
        </ng-template>
